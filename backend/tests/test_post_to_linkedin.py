...
@pytest.mark.parametrize(
    "linkedin_id, expected_urn",
    [
        ("123456", "urn:li:member:123456"),
        ("urn:li:member:123456", "urn:li:member:123456"),
    ],
)
def test_post_to_linkedin_formats_author_correctly(
    ...
    def mock_post(url, json, headers):
        ...
        return SimpleNamespace(
            status_code=201, json="{'id': 'fake-post-id'}", text="OK"
        )
...
def test_post_to_linkedin_success():
    ...
    with requests_mock.Mocker() as m:
        m.post(
            ...
            json={"id": "mock_post_id"},
            status_code=201,
        )

        result = post_to_linkedin(
            user,
            "ajharris/github-linkedin-auto-post",
            "Commit message",
            webhook_payload,
        )
        ...
        assert (
            request_payload["author"] == "urn:li:member:123456789"
        ), "Author URN should use 'person', not 'member'"
...
def test_post_to_linkedin(mock_generate_post, mock_requests_post):
    ...
    response = post_to_linkedin(
        mock_user, "test-repo", "Initial commit", webhook_payload
    )
...
def test_send_post_to_linkedin_refreshes_token(app):
    ...
    with app.app_context():
        ...
        with patch(
            "backend.services.post_to_linkedin.exchange_code_for_access_token",
            return_value="new_mock_token",
        ) as mock_refresh_token, patch(
            "backend.services.post_to_linkedin.post_to_linkedin"
        ) as mock_post_to_linkedin:

            mock_post_to_linkedin.return_value = MagicMock(
                status_code=201, json=lambda: {"id": "mock_post_id"}
            )

            response = send_post_to_linkedin(
                user, "test-repo", "Test commit message", {}
            )
