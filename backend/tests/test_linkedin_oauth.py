...
def github_user(db_session):
    user = User(github_id="12345", github_username="testuser", github_token="mock_token")  # Added github_token
    db.session.add(user)
    db.session.commit()
    return user
...
def test_linkedin_callback_stores_token_and_urn(mock_get, mock_post, client, db_session, github_user):
    ...
    response = client.get("/auth/linkedin/callback?code=fake_code&state=xyz")
    assert response.status_code == 302

    updated_user = db.session.get(User, github_user.id)
    assert updated_user.linkedin_access_token == "mock_access_token"
    assert updated_user.linkedin_user_urn == "urn:li:person:abcd1234"

def test_user_model_has_linkedin_fields(github_user):
    assert hasattr(github_user, "linkedin_access_token")
    assert hasattr(github_user, "linkedin_user_urn")

def test_linkedin_status_endpoint(client, github_user, db_session):
    github_user.linkedin_access_token = "token123"
    github_user.linkedin_user_urn = "urn:li:person:abcd"
    db_session.commit()
